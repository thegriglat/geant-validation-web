...
void Record::initializeTH1F()
{
    ...
    size_t nBins = 0;
    if (!labels_enable) {
        auto  v_binMin = json2iterable<float>(data["histogram"]["binEdgeLow"]);
        ...
        InitTH1F("TH1F", "TH1F", v_binMin.size() - 1, &v_binMin[0]);
        nBins = v_binMin.size();
    } else {
...
vector<float> Record::mergeErrors(const json &obj, const string axis, const string dir) const
{
    ...
    if (eesys.size() == 0) return eestat;
    if (eestat.size() == 0) return eesys;
